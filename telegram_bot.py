...
/stop - Dừng đánh cờ (Vẫn giữ kết nối Telegram).
/cancel - Hủy lời mời thách đấu đang treo.
/shutdown - Tắt hoàn toàn chương trình trên Termux.
...
def start_handler(message):
    ...
    if not lichess_bot or not lichess_bot.running:
        bot.reply_to(message, "🚀 Đang bật bot...")
    ...
    else: bot.reply_to(message, "⚠️ Bot đang chạy rồi.")
...
def stop_handler(message):
    ...
    if lichess_bot:
        ...
        lichess_bot = None
        bot.reply_to(message, "🛑 Đã dừng đánh cờ.")
...
def resetstats_handler(message):
    ...
    if os.path.exists(path):
        ...
        # Nếu bot đang chạy, reset đối tượng Stats trong RAM
        if lichess_bot:
        ...
        bot.reply_to(message, "ℹ️ Hiện chưa có dữ liệu thống kê để xóa.")
...
def stats_handler(message):
    ...
    bot.send_message(admin_id, f"📊 *THỐNG KÊ:*\n🔢 Tổng: `{d['games']}`\n🏆 Thắng: `{d['wins']}` ({wr:.1f}%)\n😔 Thua: `{d['losses']}`\n🔥 Chuỗi hiện tại: `{d['current_streak']}`", parse_mode="Markdown")
